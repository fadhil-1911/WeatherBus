...
bool setupSHT41() {
    ...
    if (Wire.endTransmission() != 0) {
        Serial.println(
            "ERROR: SHT41 communication failed"
        );
        return false;
    }
...
bool readSHT41(float& temperature, float& humidity) {
    ...
    if (Wire.endTransmission() != 0) {
        Serial.println(
            "ERROR: SHT41 measurement command failed"
        );
        return false;
    }
    // High precision measurement requires
    // approximately 8 ms.
    delay(10);
    // -------------------------------------------------
    // Request 6 bytes
    ...
    if (received != 6) {
        Serial.println(
            "ERROR: SHT41 invalid response length"
        );
        return false;
    }
    ...
    if (sht41Crc8(data, 2) != data[2]) {
        Serial.println(
            "ERROR: SHT41 temperature CRC failed"
        );
        return false;
    }
    ...
    if (sht41Crc8(&data[3], 2) != data[5]) {
        Serial.println(
            "ERROR: SHT41 humidity CRC failed"
        );
        return false;
    }
...
void sendSensorData(
    ...
    // Send packet
    // -------------------------------------------------
    esp_err_t result = esp_now_send(baseMac, reinterpret_cast<uint8_t*>(&packet), sizeof(packet));
    if (result == ESP_OK) {
        Serial.printf(
...
void onDataReceived(const uint8_t* mac, const uint8_t* data, int len) {
    ...
    // =================================================
    // Copy packet into Header structure
    // =================================================
    Header request{};
    ...
    if (request.packetType != static_cast<uint8_t>(PacketType::DATA_REQUEST)) {
        Serial.printf(
            "RX ERROR: Invalid packet type | Received=0x%02X\n",
            request.packetType
        );
        return;
    }
    ...
    if (request.nodeId != NODE_ID) {
        Serial.printf(
            "RX ERROR: Invalid Node ID | Received=%u | This Node=%u\n",
            request.nodeId,
            NODE_ID
        );
        return;
    ...
    //
    // DATA_REQUEST has no payload.
    // Therefore payloadLength must be 0.
    // =================================================
    if (request.payloadLength != 0) {
        Serial.printf(
            "RX ERROR: Invalid payload length | Received=%u | Expected=0\n",
